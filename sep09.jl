...
#Date Sep 09 2021 
#
    #' Check how to import the file- download data into local folder?
...
dat = readdlm("/Users/jiwonpark/Documents/GitHub/githubTest
/USairpollution.csv", ',', Any, '\n')
#' Export dat
...
Random.seed!(2021); # set seed of random
    #' Allow us to replicate the same results
...
a = rand(1:9, 9)
sort(a)
sort!(a) #' change a by sorting order
...
for x in 1:10    # x in collection, can also use = here instead of in
    ...
    #' Check the results:: Cann't understand why 1 to 10 appears
...
ρ.^abs.(1(1:p) .- (1:p)') #' Do not necessarily put 1 in front of (1:p)
ρ.^abs.((1:p) .- (1:p)')
...
sum_global()        #' Didn't provide time information
@time sum_global()  #' We could get spending time from this code
...
# 
    ...
    ##' Need to check why RCall didn't work
    using Pkg 
    Pkg.add("RCall")
    Pkg.build("RCall")
    using RCall
...
ols = (X'X) \ (X'y) # it calculates (X' * X) \ (X' * y)
#' or simply
X \ y   ## if our (X'X) is not unique then this is not applicable
#' Do NOT use 
inv(X' * X) * X' * y
    ...
    R"z <- y * 3"
    @rget z
    #' Note that z is R variables. @rget sends z to julia with the same names
    

#+ eval=false; echo = false; results = "hidden"
Pkg.add("Weave")
using Weave
weave("note02_code_short.jl", cache=:off, doctype="github")
weave("note02_code_short.jl", cache=:off, out_path="output/")
weave("note02_code_short.jl", cache=:off)


## 2021. Sep 14
## Check the directory of the file and github path
## RCall doesn't work
    ## Julia -> R : shift + $
    ## R -> Julia : backspace
    ## julia> R"R.code"
## Several comments I wrote that need to be checked later
